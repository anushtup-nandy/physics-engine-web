...
function createBlock(){
    ...
    let pos = {x: 0, y: 0, z: 0};
    let scale = {x: 50, y: 2, z: 50}; // Adjust these dimensions as needed
    let quat = {x: 0, y: 0, z: 0, w: 1};
    let mass = 0;

    // Specify dimensions for the box geometry
    let blockGeometry = new THREE.BoxBufferGeometry(scale.x, scale.y, scale.z);

    // Create mesh with specified geometry and material
    let blockPlane = new THREE.Mesh(blockGeometry, new THREE.MeshPhongMaterial({color: 0xa0afa4}));
...
function createJointObjects(){
    ...
    //Block Graphics
    let blockGeometry = new THREE.BoxBufferGeometry(scale.x, scale.y, scale.z);
    let block = new THREE.Mesh(blockGeometry, new THREE.MeshPhongMaterial({color: 0xf78a1d}));
